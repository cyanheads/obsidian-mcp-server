...
- **A heading at or above the target section's own level in written content is rejected** with the new `heading_outside_section` reason instead of silently closing the section early.
...
- **A failing `GET /` now fails the write with its classified error** instead of guessing a markdown-patch format.
...
- **`obsidian_get_note` `format: "section"` finds headings the way the document map does** — lexed with `marked`, so setext headings count and a `#` line inside a list item or HTML block does not — so a section read now covers exactly the span a write edits ([#141](https://github.com/cyanheads/obsidian-mcp-server/issues/141)).
