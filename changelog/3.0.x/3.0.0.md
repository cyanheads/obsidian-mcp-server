...
| `obsidian_list_notes` | Same name. Same extension and name-regex filters; result echoes `appliedFilters`. Returns separate `files[]` and `directories[]` arrays — v2's nested tree-view rendering is gone. |
...
- **Read tools:** `obsidian_get_note` (content / full / document-map / section projections), `obsidian_list_notes` (with extension and name-regex filters), `obsidian_list_tags`, `obsidian_list_commands`, `obsidian_search_notes` (text / Dataview DQL / JSONLogic).
