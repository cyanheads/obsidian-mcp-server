...
- `engines.bun` ≥ 1.3.11. Node ≥ 22 still works at runtime; the install-time constraint is Bun-pinned because the dev workflow (build, test, devcheck) assumes it.
...
- `engines.bun` tightened from `>=1.2.0` to `>=1.3.11` to match the pinned `packageManager` and documented prerequisite. Older Bun runtimes no longer satisfy the install constraint.
