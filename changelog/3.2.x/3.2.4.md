---
summary: "mcp-ts-core ^0.9.16 → ^0.9.21 (per-request log context, retryable auto-population, query-string redaction, devcheck gates); plugin marketplace support (.claude-plugin, .codex-plugin); MCP config key renamed to obsidian-mcp-server; OBSIDIAN_DENY_PATHS adds denylist path policy support"
breaking: false
security: false
agent-notes: |
...
- **`OBSIDIAN_DENY_PATHS`** — comma-separated vault-relative folder denylist for read and write operations. Denylist matches use the same normalized prefix rules as `OBSIDIAN_READ_PATHS` / `OBSIDIAN_WRITE_PATHS`, win over allowlists, and return `path_forbidden` / `denied_path` with the denylist echoed in `data.activeScope`.

## Changed

 - **Path-policy docs** now call out that folder listings are navigational: allowed listings can reveal denied or out-of-scope directories as immediate children, those directories are marked `truncated` and not walked, and direct listing, reads, or writes inside them still throw `path_forbidden`.
